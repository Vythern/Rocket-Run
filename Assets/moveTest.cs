...
//Feb 7th, 2024.  
//Made it so that the crosshair is disabled until the script is initalized, so that pressing f takes you to the player model instead of the skybox.  
...
public class moveTest : MonoBehaviour
{
    ...
    [SerializeField] private GameObject crosshair; //put weapons on the viewModel layer to make them appear to the player, if they are not, they will clip.  pickups and props should not be on viewmodel layer.  
    ...
    public void applyKnockBack(Vector3 knockBackVector) { playerVelocity += knockBackVector; }

    public Vector3 returnVelocity() { return playerVelocity; }
    ...
    private void OnEnable() //this runs when the player is instantiated.  
    {
        //right now, all it does is make the crosshair visible (because it is disabled by default, so as to allow teleporting close to the player model in edit mode).  
        //but it could also potentially set the player's ammo, health, other characteristics, etc, from a "save" file?  
        //perhaps that's more important to have on the weapons control script.  
        //and maybe that's unneeded because the save could send a message to set these parameters itself to the player object.  
...
//weaker air movement force, weaker air friction?  
//we want rocket jumping to be more useful, (max of 30 something speed from jumping, which is double ground speed, but marginally better than max jump speed.  
//perhaps adjust gravity to be weaker / slower...  
//or maybe we have some special velocity to track, "knockback velocity," which is not affected the same way as friction.  